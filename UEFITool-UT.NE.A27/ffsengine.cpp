...
void FfsEngine::init(){
	file.setFileName("CapsuleGUID.txt");
	file1.setFileName("FileSystemGUID.txt");
	file2.setFileName("SectionGUID.txt");
	file3.setFileName("FileGUID.txt");
...
UINT8 FfsEngine::parseImageFile(const QByteArray & buffer)
{
	...
	if(!check111[guidToQString(capsuleHeader->CapsuleGuid)]){	
		out << guidToQString(capsuleHeader->CapsuleGuid) << "\n";
		check111[guidToQString(capsuleHeader->CapsuleGuid)] = 1;
	}
//	qDebug() << guidToQString(capsuleHeader->CapsuleGuid);
...
//    qDebug() << "FileSystemGuid : " << guidToQString(volumeHeader->FileSystemGuid);
...
UINT8 FfsEngine::parseFile(const QByteArray & file, QModelIndex & index, const UINT8 erasePolarity, const QModelIndex & parent, const UINT8 mode)
{
	...
	if(!check111[name]){
		out << guidToQString(fileHeader->Name) << "\n";
		check111[name] = 1;
	}
//	qDebug() << "File GUID : " << guidToQString(fileHeader->Name);
    info = tr("File GUID: %1\nType: %2h\nAttributes: %3h\nFull size: %4h (%5)\nHeader size: %6h (%7)\nBody size: %8h (%9)\nState: %10h")
...
UINT8 FfsEngine::parseSection(const QByteArray & section, QModelIndex & index, const QModelIndex & parent, const UINT8 mode)
{
	...
	if(!check111[name]){
		out << guidToQString(guidDefinedSectionHeader->SectionDefinitionGuid) << "\n";
		check111[name] = 1;
	}
//	qDebug() << "SectionGuid : " << name;
